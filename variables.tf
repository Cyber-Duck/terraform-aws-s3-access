...
variable "s3_arns" {
  ...
  validation {
    condition     = can(regex("^[arn:aws:s3:::a-z-\\.]+$", contains(flatten(tolist(var.s3_arns))))
...
variable "additional_policy_arn" {
  ...
  default     = null
  validation {
    condition     = can(regex("^[arn:aws:a-z-\\.]+$", contains(flatten(tolist(var.additional_policy_arn))))

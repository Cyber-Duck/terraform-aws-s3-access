variable "iam_user" {
  ...
  validation {
    condition     = var.iam_user == null || can(regex("^[0-9A-Za-z+=,.@_-]+$", var.iam_user))
...
variable "iam_group_name" {
  ...
  default     = null
  validation {
    condition     = var.iam_group_name == null || can(regex("^[0-9A-Za-z+=,.@_-]+$", var.iam_group_name))
...
variable "existing_group" {
  ...
  default     = null
  validation {
    condition     = var.existing_group == null || can(regex("^[0-9A-Za-z+=,.@_-]+$", var.existing_group))
...
variable "additional_policy_arn" {
  ...
  default     = null
  validation {
    condition     = var.additional_policy_arn == null || can(regex("^[arn:aws:a-z-\\.]+$", [for arns in var.additional_policy_arn : arns.arn]))
